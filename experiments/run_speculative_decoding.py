...
def benchmark_speculative_generation(target_adapter: ModelAdapter, draft_adapter: ModelAdapter, input_batch: torch.Tensor) -> dict:
    """Benchmark the model's latency and throughput on the given input batch."""
    target_adapter.use_cache = True
    draft_adapter.use_cache = True

    target_cache = {"past": None}
    draft_cache = {"past": None}

    def clear_past_cache(cache : dict, layer_idx):
        def tmp(*_):
            if cache["past"]:
    ...
    layers = target_adapter.get_layers()
    ...
    layers = draft_adapter.get_layers()
    for idx, layer_adapter in enumerate(layers):
        # Clear past cache after each layer gets called to get accurate timing of each forward pass.
        layer_adapter.layer.register_forward_hook(clear_past_cache(idx))
